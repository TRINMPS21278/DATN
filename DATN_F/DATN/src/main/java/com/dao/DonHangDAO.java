...
public interface DonHangDAO extends JpaRepository<DonHang, Integer>{
	...
	@Query(value="SELECT * FROM donhang WHERE taikhoan=?1 ORDER BY ngaytao desc",nativeQuery = true)
	List<DonHang> findByIdKhachHang(String taikhoan);
	...
	@Query(value="SELECT * FROM donhang WHERE taikhoan=?1 and trangthai=?2 ORDER BY ngaytao desc",nativeQuery = true)
	List<DonHang> findByIdKHvsTT(String taikhoan, Optional<Integer> tt);
