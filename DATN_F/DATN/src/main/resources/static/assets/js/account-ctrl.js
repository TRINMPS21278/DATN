...
app.controller("account-ctrl",function($scope,$http,$location){
	...
	$scope.getOneByRole = function(taikhoan){
		$http.get(`/rest/authoritiesOne?taikhoan=${taikhoan}`).then(resp=>{
			$scope.selection = [];
	...
	$scope.edit = function(item){
		...
		$scope.getOneByRole(item.taikhoan);
		$location.path('/admin/taikhoan/edittaikhoan/' + item.taikhoan);
    }
    ...
    $scope.update = function(){
		var item = angular.copy($scope.form);
		$http.put(`/rest/accounts/${item.taikhoan}`,item).then(resp=>{
			var index = $scope.items.findIndex(p=>p.taikhoan == item.taikhoan);
			$scope.items[index] = item;
			
			//xoá toàn bộ roles của user hiện tại
			$http.delete(`/rest/authoritiesOne/${item.taikhoan}`).then(resp=>{
    ...
    $scope.delete = function(item){
		$http.delete(`/rest/accounts/${item.taikhoan}`).then(resp=>{
			var index = $scope.items.findIndex(p=>p.taikhoan == item.taikhoan);
